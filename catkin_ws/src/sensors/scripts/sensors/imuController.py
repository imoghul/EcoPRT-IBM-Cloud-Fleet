...
class IMUController:
    ...
    def calibrate(self, newRaw, samples):
        rospy.loginfo(
            "CALIBRATNG IMU..." + str(100 * self.calibrateCounter / samples) + "%"
        )
        # sumAx = []
        # sumAy = []
        # sumAz = []
        ...
        Ax, Ay, Az, Gx, Gy, Gz = (
            newRaw.linear_acceleration.x,
            newRaw.linear_acceleration.y,
            newRaw.linear_acceleration.z,
            newRaw.angular_velocity.x,
            newRaw.angular_velocity.y,
            newRaw.angular_velocity.z,
        )
    ...
    def calc(self, newRaw):
        ...
        if self.refereshIMUData(newRaw) == False:
            return
        if (
            self.data.Ax == origAx
            and self.data.Ay == origAy
            and self.data.Az == origAz
            and self.data.Gx == origGx
            and self.data.Gy == origGy
            and self.data.Gz == origGz
        ):
